    ...
    const fetchModules = async () => {
      ...
      if (!error && data) {
        ...
        const mods = await Promise.all(data.map(async (m) => {
          const { count: lectureCount } = await supabase.from('lectures').select('*', { count: 'exact', head: true }).eq('module_slug', m.slug).eq('published', true);
          const { count: labCount } = await supabase.from('labs').select('*', { count: 'exact', head: true }).eq('module_slug', m.slug).eq('published', true);
          return {
            ...m,
