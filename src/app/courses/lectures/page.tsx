...
export default function LecturesPage() {
  ...
  const allLectures = modulesData.flatMap(mod => 
    mod.lectures.filter(l => l.type === 'reading' || l.type === 'video').map(lecture => ({ module: mod, lecture }))
  );

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 pt-24 min-h-screen">
      <h1 className="text-3xl font-bold text-[var(--foreground)] mb-8">
        {t("All Lectures", "Все лекции")}
      </h1>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {allLectures.map(({ module: mod, lecture }) => (
          <Link
          ...
          >
            <div className="relative h-36 overflow-hidden">
              ...
              <div className="absolute top-2 right-2">
                <span className="px-2 py-0.5 rounded-md text-xs font-semibold text-white bg-gray-600">
                  {lecture.type}
                </span>
              </div>
            </div>
